...
contains
    ...
    impure subroutine s_save_data(t_step, varname, pres, c, H)
        ...
        real(wp), dimension(-offset_x%beg:m + offset_x%end, &
                            ...
                            -offset_z%beg:p + offset_z%end) :: liutex_mag, vort_stretch_proj, vort_stretch_res, liutex_mag_filtered
        
        real(wp), dimension(-offset_x%beg:m + offset_x%end, &
                            -offset_y%beg:n + offset_y%end, &
                            -offset_z%beg:p + offset_z%end) :: A_rr, A_ps, A_ns, A_sr
        ...
        real(wp), dimension(0:m_glb, 0:n_glb, 0:p_glb) :: vel1_glb, vel2_glb, vel3_glb
...
!             call s_mpi_barrier()
            ...
            do k = -offset_z%beg, p + offset_z%end
                do j = -offset_y%beg, n + offset_y%end
                    if (y_cc(j) >= y_cc_glb(mixlayer_idx_beg) .and. y_cc(j) <= y_cc_glb(mixlayer_idx_end)) then
                        do i = -offset_x%beg, m + offset_x%end
                            q_sf(i, j, k) = 0._wp
